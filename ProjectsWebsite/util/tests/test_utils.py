...
from ProjectsWebsite.util.tests.utils import _temp_save_expected_responces, _pofile_expected_translations
    ...
    for _ in range(2):
        ...
        if json and json["type"] == "test":
            if res.status_code == 200 or res.status_code == 500:
                ...
                pytest.fail(f"The url did not return a 200 or 500 status code, Instead: {res.status_code}")
...
def test_temp_save():
    ...
    # assertions
    assert temp_save["type"] == _temp_save_expected_responces["type"], 'temp_save["type"] does not equal  _temp_save_expected_responces["type"]'
    assert temp_save["number"] == _temp_save_expected_responces["number"], 'temp_save["number"] does not equal  _temp_save_expected_responces["number"]' 
    
def test_temp_save_setMultipleValues():
    # tests the setMultipleValues from the temp_save dict
    temp_save = _temp_save()
    temp_save.setMultipleValues(("Numbers", "str", "line", "test"), (1, 2, 3, 4), "This is a test string", "foo bar", "temp_save")
    assert (temp_save["Numbers"], temp_save["str"], temp_save["line"], temp_save["test"]) == (_temp_save_expected_responces["multivalues"]["Numbers"], _temp_save_expected_responces["multivalues"]["str"],
                                                                                              _temp_save_expected_responces["multivalues"]["line"], _temp_save_expected_responces["multivalues"]["test"]), 'temp_save values set by setMultipleValues did not match the values stored inside of _temp_save_expected_responces["multivalues"]'
...
def test_PoFileAutoTranslator():
    ...
    for entry in po:
        assert entry.msgstr == _pofile_expected_translations[entry.msgid], f'''{entry.msgstr} does not equal 
...
def test_unverfiedLogUtil():
    ...
    test_string = ("Exerpt", "This is a test excerpt",)
    test_log.addContent(test_string)
    with open(_log_dir, "r", encoding="utf-8") as log:
        lines = log.readlines()
        log.close()
    assert lines != [] and lines[0] == f"({test_string[0]}) {test_string[1]}", "The log was blank or the log line did not match the test_string"
...
def test_AlertUtil():
    ...
    assert test_alert.getConfigValue("ALERT_CODES_NUMBER_LIST") == ["0", "1", "2", "3", "4"], "function AlertUtils.getConfigValue did not get the correct value for ALERT_CODES_NUMBER_LIST"
    
    test_alert.setAlert('info', "This is a test alert.")
    assert test_alert.alert_dict == {"type": "info", "message": "This is a test alert."}, "The test_alert dictionary does equal to what it is supposed to."
    
    test_alert_dict = test_alert.getAlert()
    assert test_alert_dict["Type"] == "info" and test_alert_dict["Msg"] == "This is a test alert.", "One of the values in the dict is not correct."
...
def test_is_valid_article_page(app):
    ...
    assert res.status_code == 200, f'''Test Client did not return a 200 http status. returned: {res.status_code}. Traceback if there is one:
    ...
    assert res.status_code == 404, f'''Test client did not return a 404 http status but returned {res.status_code}. Traceback if there is one:
                                    Type: {sys.exc_info()[0] if sys.exc_info() else "None"}
                                    Traceback: {traceback.format_exception_only(sys.exc_info()[0], sys.exc_info()[1]) if sys.exc_info() else "None"}'''
